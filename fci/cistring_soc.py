#!/usr/bin/env python
# This module defines functions for generating cstrings for SOC type objects
...
def num_strings_soc(n, m):
    if m < 0 or m > n:
	return 0
    else:
	return (sum([num_strings(n,a) for a in range(m+1)]))
...
#Same idea as regular index table
#but now we must map to different particle sector
# must provide neleca, nelecb 
# Note method requires knowledge of aorder:
# aorder = 0: c+_a c_b
# aorder = 1: c+_b c_a 
# which enables correct mapping.
# Note target address is in the |Na+1,Nb-1> or |Na-1,Nb+1> block and not as absolute address
# Absolute address will need to be adjusted in callers. It is done this way for convenience
# look at contract_1esoc_soc for example

def gen_linkstr_index_o0_soc(orb_list, neleca, nelecb, aorder,  strs=None):

    ns = len(orb_list) 

    if strs is None:
	if aorder == 0:
	        strsb = gen_strings4orblist(orb_list, nelecb)
		strsa = gen_strings4orblist(orb_list, neleca)	
	        strsap = gen_strings4orblist(orb_list, neleca+1)
	        strsbm = gen_strings4orblist(orb_list, nelecb-1)
	else:
	        strsb = gen_strings4orblist(orb_list, neleca)
	        strsa = gen_strings4orblist(orb_list, nelecb)
	        strsap = gen_strings4orblist(orb_list, nelecb+1)
	        strsbm = gen_strings4orblist(orb_list, neleca-1)

    strdic = dict(zip(strsbm,range(strsbm.__len__())))
    strdict = dict(zip(strsa,range(strsa.__len__())))

    def pump1e(str0):

        occ = []
        vir = []
	#Gather states that are occupied or empty
        for j in orb_list:
            if str0 & (1<<j):
                occ.append(j)
		
        for pos,str1 in enumerate(strsap):
	    for i in orb_list:
		if str1 & (1<<i):
		    vir.append((pos,i))
	...
	srcvec = []
        for j in occ:
            for p,i in vir:
		strt = str0 ^ (1<<j) #new string

                # [cre, des, target_address, parity]
		target = [0,0]
		target[aorder] = p 
		target[1-aorder] = strdic[strt]

		#For parity write out full string
		#order of string requires handling
		#bit string for b appears after a
		#Recall aorder = 0 => c+_ia c_jb
		strx = strsap[p] ^ (1<<i) 	
